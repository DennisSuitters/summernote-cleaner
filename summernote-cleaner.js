...
(function($){
  ...
  $.extend($.summernote.options,{
    cleaner:{
      ...
      badTags:['col','colgroup','title','style','script','applet','embed','noframes','noscript'], //Remove full tags with contents
      badAttributes:['bgcolor','border','width','height','cellpadding','cellspacing','valign','lang','style','start'], //Remove attributes from remaining tags
  ...
  $.extend($.summernote.plugins,{
    'cleaner':function(context){
        ...
        context.memo('button.cleaner',function(){
          var button=ui.button({
            ...
            click:function(){
              ...
              if ($editor.find('.note-status-output').length > 0)$editor.find('.note-status-output').html(lang.cleaner.not);
            }
          });
      ...
      var cleanPaste = function(input,badTags,badAttributes){
        ...
        output=output.replace(/<b>/gi,'<strong>');output=output.replace(/<\/b>/gi,'</strong>');
        output=output.replace(/<i>/gi,'<em>');output=output.replace(/<\/i>/gi,'</em>');
